...
const steps = [
  ...
  },
  {
    ...
    desc: "Post-production complète : montage, étalonnage, sound design, motion graphics. Deux rounds de retours clients inclus. Déclinaisons tous formats livréesà la fin.",
    details: [
      "Montage narratif",
            ...
            {steps.map((step, i) => (
              ...
              >
                <div className="flex flex-col items-center">
                  <div
                    className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border border-border"
                  >
                    <span className="font-sora font-light text-sm text-mid">
                      {step.num}
                    </span>
                  </div>
                  {i < steps.length - 1 && (
                    <div className="flex-1 w-px mt-3 bg-border" />
                  )}
                </div>
                <div className="pb-4">
                  <p className="font-mono text-[9px] uppercase tracking-wider text-light mb-2">
                    {step.label}
                  </p>
                  <h2 className="font-sora font-light text-[28px] text-black mb-3 leading-snug">
                    {step.title}
                  </h2>
                  <p className="font-sora font-light text-sm text-mid leading-relaxed mb-5">
                    {step.desc}
                  </p>
                  <ul className="flex flex-col gap-2">
                    {step.details.map((d, j) => (
                      <li key={j} className="flex items-center gap-2.5">
                        ...
                        <span className="font-sora font-light text-sm text-mid">{d}</span>
                      </li>
                    ))}
                  </ul>
                </div>
